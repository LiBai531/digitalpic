...
static T_Layout g_atManualMenuIconsLayout[] = {
    ...
    {0, 0, 0, 0, "select.bmp"},//new icon for filter, use select.bmp for a while
    {0, 0, 0, 0, "select.bmp"},//new icon for filter, use select.bmp for a while
    {0, 0, 0, 0, "select.bmp"},//new icon for filter, use select.bmp for a while
    {0, 0, 0, 0, "select.bmp"},//new icon for filter, use select.bmp for a while
	{0, 0, 0, 0, NULL},
...
static void  CalcManualPageMenusLayout(PT_PageLayout ptPageLayout)
{
	...
	else
	{
        ...
        //filter icons
        atLayout[6].iTopLeftX = iWidth + 1;
        atLayout[6].iTopLeftY = (iYres - iHeight*4) / 2 - 1;
        atLayout[6].iBotRightX = atLayout[6].iTopLeftX + iWidth - 1;
        atLayout[6].iBotRightY = atLayout[6].iTopLeftY + iHeight - 1;
        //and other filter icons
        for (i = 7; i < 10; i++)
        {
    		atLayout[i].iTopLeftY  = atLayout[i-1].iBotRightY+ 1;
    		atLayout[i].iBotRightY = atLayout[i].iTopLeftY + iHeight - 1;
    		atLayout[i].iTopLeftX  = iWidth + 1;
    		atLayout[i].iBotRightX = atLayout[i].iTopLeftX + iWidth - 1;
        }
	}
...
static void CalcManualPagePictureLayout(void)
{
	...
	else
	{
		 ...
		 *	  --------------------------------------------------------------
		 */
		iTopLeftX  = g_atManualMenuIconsLayout[6].iBotRightX + 1;
		iBotRightX = iXres - 1;
		iTopLeftY  = 0;
		iBotRightY = iYres - 1;
	}
...
static int ShowPictureInManualPage(PT_VideoMem ptVideoMem, char *strFileName)
{
    ...
    if (1 == filter)
    {
        ...
        int bytes_num = ptOriginPicPixelDatas->iTotalBytes;
        
        unsigned short n565Color;
        //unsigned int n888Color;
        for(i=0; i < bytes_num; i+=2)
        {
            n565Color = (ptOriginPicPixelDatas->aucPixelDatas[i] << 8) + (ptOriginPicPixelDatas->aucPixelDatas[i+1] << 0);
            unsigned int cRed   = n565Color >> 11;
            unsigned int cGreen = (n565Color >> 5) & 0x3f;
            unsigned int cBlue  = n565Color & 0x1f;
            
            unsigned int cGray = cRed*0.3 + cGreen*0.59 + cBlue*0.11;
            
            n888Color = (cGray << 16) + (cGray << 8) + (cGray << 0);
            printf("Origin n565Color: %x",n565Color);
            printf("r: %x",cRed);
            printf("g: %x",cGreen);
            printf("b: %x",cBlue);
            printf("gray: %x", cGray);
            printf("n888Color: %x", n888Color);
            unsigned char iRed   = cGray >> 3;
            unsigned char iGreen = cGray >> 2;
            unsigned char iBlue  = cGray >> 3;
            
            n565Color = (iRed << 11) + (iGreen << 5) + (iBlue << 0);
            //printf("Gray n565Color: %x",n565Color);
            //printf("high 8 bits: %x",((n565Color & 0xff00) >> 8));
            //printf("high 8 bits: %x",((n565Color & 0x00ff) >> 0));
            ptOriginPicPixelDatas->aucPixelDatas[i] = (n565Color & 0xff00) >> 8;
            ptOriginPicPixelDatas->aucPixelDatas[i+1] = n565Color & 0x00ff;
